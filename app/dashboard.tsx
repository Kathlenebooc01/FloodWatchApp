...
export default function Dashboard() {
    ...
    useEffect(() => {
        (async () => {
            try {
                console.log('🏠 Dashboard: Fetching location...');
                // Clear cache to get fresh location
                clearLocationCache();
                const addr = await getCurrentFullAddress();
                console.log('🏠 Dashboard: Location received:', addr.full);
                setFullAddress(addr.full);
                setCityName(addr.city);
            } catch (error: any) {
                console.error('🏠 Dashboard: Location error:', error?.message);
                if (error?.message === 'PERMISSION_DENIED') {
                    setFullAddress('Location permission denied');
                } else if (error?.message === 'GPS_TIMEOUT') {
                    setFullAddress('GPS signal weak, please wait...');
                    ...
                    setTimeout(async () => {
                        try {
                            ...
                            setFullAddress('Unable to get location');
                        }
                    }, 2000);
                } else if (error?.message === 'GEOCODE_TIMEOUT') {
                    setFullAddress('Getting address details...');
                    // Retry immediately since GPS worked, just geocoding was slow
                    setTimeout(async () => {
                        try {
                            clearLocationCache();
                            const addr = await getCurrentFullAddress();
                            ...
                            setFullAddress('Cebu City'); // At least show city name
                        }
                    }, 1000);
                } else {
        ...
        // Fetch REAL weather data from API (initial load)
        (async () => {
        ...
        // Fetch risk status (initial load)
        fetchRiskStatus();

        // Auto-refresh real weather and risk status every 1 minute (60 seconds)
